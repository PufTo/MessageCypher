...
console.log(messageInput, secretCodeInput, encryptButton, decryptButton, copyButton);
...
const decryptMessage = (event)=>{
    ...
    const encryptedMessage = messageToEncrypt.split('').map((character, index) => {
        ...
        if (descryptedIndex < 0)
            decryptedIndexFinal += validCharacters.length;
